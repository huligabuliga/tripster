...
const NewExpense = () => {
  ...
  const initializePayees = (groupMembers) => {
    if (!groupMembers) {
      console.error("Group members data is not available");
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/api/expenses/register",
        {
          name: name,
          categories: categories.map((c) => c.value),
          currency: currency.value,
  ...
  return (
    <div className="w-full min-h-screen">
      {/** Header */}
      <h1 className="text-3xl font-bold my-4 text-center">New Expense</h1>
      ...
      >
        ...
        {/** Paid by */}
        <div className="">
          <h2 className="font-semibold text-xl">Paid by</h2>

          <Select
            options={payees.map((mem) => ({
              label: mem.username,
              value: mem._id,
            }))}
            value={payer}
